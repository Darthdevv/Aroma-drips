...
const MilkShakeProducts = (): JSX.Element => {
    ...
    const handleKeyDown = (e: React.KeyboardEvent, item: typeof MilkshakesItems[0]) => {
        ...
        if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(e.key)) {
            ...
            switch (e.key) {
                case 'ArrowRight':
                    nextIndex = (currentIndex + 1) % MilkshakesItems.length;
                    break;
                case 'ArrowLeft':
                    nextIndex = (currentIndex - 1 + MilkshakesItems.length) % MilkshakesItems.length;
                    break;
                case 'ArrowDown':
                    nextIndex = Math.min(currentIndex + itemsPerRow, MilkshakesItems.length - 1);
                    break;
                case 'ArrowUp':
