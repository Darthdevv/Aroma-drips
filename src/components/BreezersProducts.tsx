...
const BreezersProducts = (): JSX.Element => {
    ...
    const handleKeyDown = (e: React.KeyboardEvent, item: typeof BreezersItems[0]) => {
        ...
        if (['ArrowUp', 'ArrowDown', 'ArrowLeft', 'ArrowRight'].includes(e.key)) {
            ...
            switch (e.key) {
                case 'ArrowRight':
                    nextIndex = (currentIndex + 1) % BreezersItems.length;
                    break;
                case 'ArrowLeft':
                    nextIndex = (currentIndex - 1 + BreezersItems.length) % BreezersItems.length;
                    break;
                case 'ArrowDown':
                    nextIndex = Math.min(currentIndex + itemsPerRow, BreezersItems.length - 1);
                    break;
                case 'ArrowUp':
